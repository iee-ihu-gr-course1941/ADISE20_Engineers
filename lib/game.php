...
function update_game_status() {
	...
	$st3=$mysqli->prepare('select count(*) as aborted from players WHERE last_action< (NOW() - INTERVAL 3 MINUTE)');
	$st3->execute();
	$res3 = $st3->get_result();
	$aborted = $res3->fetch_assoc()['aborted'];
	if($aborted>0) {
		$sql = "UPDATE players SET username=NULL, token=NULL WHERE last_action< (NOW() - INTERVAL 3 MINUTE)";
	...
	$st = $mysqli->prepare($sql);
	$st->bind_param('ss',$new_status,$turn);
	$st->execute();
