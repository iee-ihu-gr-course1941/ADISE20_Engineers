...
function show_users() {
	global $mysqli;
	$sql = 'select username, s_color from players';
...
function show_user($s_color) {
	global $mysqli;
	$sql = 'select username, s_color from players where s_color=?';
	$st = $mysqli->prepare($sql);
	$st->bind_param('s', $s_color);
	$st->execute();
	$res = $st->get_result();
	header('Content-type: application/json');
	print json_encode($res->fetch_all(MYSQLI_ASSOC), JSON_PRETTY_PRINT);
...
function set_user($b,$input) {
	...
	update_game_status();
	$sql = 'select * from players where s_color=?';
	$st = $mysqli->prepare($sql);
	$st->bind_param('s',$b);
	$st->execute();
	$res = $st->get_result();
